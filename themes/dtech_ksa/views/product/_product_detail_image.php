...
if (!empty($product->productProfile[0])) {
    if (!empty($product->productProfile[0]->productImages[0])) {
        ...
        echo CHtml::link($detail_img, $product->productProfile[0]->productImages[0]->image_url['image_gallery'], array("rel" => 'lightbox[_default]',));
    } else {
    ...
    foreach ($product->productProfile[0]->productImages as $img) {
        echo CHtml::link("no", $img->image_url['image_gallery'], array("rel" => 'lightbox[_default]', 'style' => 'width:124px; height:181px'));
    }
