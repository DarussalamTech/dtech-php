...
class m140218_054724_createing_productprofile_ksa extends DTDbMigration {
    ...
    public function up() {
        ...
        $prod_prof = $this->getQueryAll("SELECT t.product_id,title,item_code,language_id,quantity,no_of_pages,binding,printing,paper,dimension,weight,weight_unit,is_shippable,edition,color,attribute,attribute,compiler_id,translator_id,is_featured,isbn 	 FROM `product_profile` as t left join product on t.product_id=product.product_id where product.city_id=" . $lhr[0]);
