...
class RightsModule extends CWebModule {
    ...
    public static function getAllowedActions() {
        ...
        $user_role = Yii::app()->db->createCommand('select itemname from authassignment WHERE userid = ' . Yii::app()->user->id)->queryRow();

        if (!empty($user_role)) {
            $sql = "SELECT  ac.child,au.child as child1 from authitemchild " .
                    " ac LEFT JOIN " .
                    " authitemchild au ON ac.child = au.parent" .
                    " WHERE ac.parent = '" . $user_role['itemname'] . "'";

            $actions = Yii::app()->db->createCommand($sql)->queryAll();

            $actions_1 = CHtml::listData($actions, "child", 'child');
            $actions_2 = CHtml::listData($actions, "child1", 'child1');
            $actions = array_merge($actions_1, $actions_2);
        }
        //  CVarDumper::dump(array_filter($actions), 10, true);  die;
